...
def downgrade_camerainfo_to_rosbag1(src: Path, dst: Path) -> None:
    ...
    register_types(
        get_types_from_msg(HEADER_NEW, 'std_msgs/msg/Header'),
        typestore,
    )
    register_types(
        get_types_from_msg(ROI, 'sensor_msgs/msg/RegionOfInterest'),
        typestore,
    )
    register_types(
        get_types_from_msg(BOOL, 'sensor_msgs/msg/boolean'),
        typestore,
    )
...
if __name__ == "__main__":
    ...
    downgrade_camerainfo_to_rosbag1(src_path, dst_path)
